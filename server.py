...
REQUIRED_CHANNELS = [
    x.strip()
    for x in os.getenv("REQUIRED_CHANNELS", ",".join(DEFAULT_CHANNELS)).split(",")
    if x.strip()
]
...
def channel_batches():
    return [REQUIRED_CHANNELS[i:i+3] for i in range(0, len(REQUIRED_CHANNELS), 3)]


def current_channel_batch_index(joined_map):
    for index, batch in enumerate(channel_batches()):
        if not all(joined_map.get(ch, False) for ch in batch):
            return index
    return len(channel_batches())


@app.get("/api/required-channels")
async def required_channels(authorization: Optional[str] = Header(default=None)):
    user, _ = get_current_user(authorization)
    user_id = ensure_user(user)

    joined_map = {}
    for channel in REQUIRED_CHANNELS:
        joined_map[channel] = await telegram_member_status(channel, user_id)

    index = current_channel_batch_index(joined_map)
    if index >= len(channel_batches()):
        return {"channels": [], "joined_all": True, "batch": index + 1, "total_batches": len(channel_batches())}

    batch = channel_batches()[index]
    return {
        "channels": [{
            "username": ch,
            "url": channel_url(ch),
            "joined": bool(joined_map.get(ch, False)),
        } for ch in batch if not joined_map.get(ch, False)],
        "joined_all": False,
        "batch": index + 1,
        "total_batches": len(channel_batches()),
    }
...
async def verify(
    ...
    conn = db()
    if index >= len(channel_batches()):
        conn.execute("UPDATE users SET joined_all=1, updated_at=? WHERE user_id=?", (now_utc(), user_id))
        referral_paid = False
        referral_row = conn.execute("SELECT id, referrer_id, status FROM referrals WHERE referred_id=? LIMIT 1", (user_id,)).fetchone()
        if referral_row and referral_row["status"] != "paid":
            referrer_id = int(referral_row["referrer_id"])
            if conn.execute("SELECT user_id FROM users WHERE user_id=?", (referrer_id,)).fetchone():
                add_balance(conn, referrer_id, REFERRAL_REWARD, "referral", "Successful referral reward", referral_row["id"])
                conn.execute("UPDATE referrals SET status='paid', reward=?, paid_at=? WHERE id=? AND status!='paid'", (REFERRAL_REWARD, now_utc(), referral_row["id"]))
                notify(conn, referrer_id, "🎉 Referral Reward", f"You earned {REFERRAL_REWARD:.2f} ETB from a successful referral.")
                referral_paid = True
        conn.commit(); conn.close()
        return {"success": True, "joined_all": True, "next_batch": False, "referral_paid": referral_paid, "message": "Verification completed.", "open_app": True}

    batch = channel_batches()[index]
    results = []
    for channel in batch:
        joined = bool(joined_map[channel])
        results.append({"channel": channel, "joined": joined})

    batch_joined = all(item["joined"] for item in results)
    conn.execute("UPDATE users SET joined_all=?, updated_at=? WHERE user_id=?", (1 if index == len(channel_batches())-1 and batch_joined else 0, now_utc(), user_id))
    conn.commit(); conn.close()

    if not batch_joined:
        return {
            "success": True, "joined_all": False, "next_batch": False,
            "channels": results,
            "missing": [x["channel"] for x in results if not x["joined"]],
            "message": "Please join all channels in this group first.",
        }

    has_next = index + 1 < len(channel_batches())
    return {
        "success": True, "joined_all": not has_next, "next_batch": has_next,
        "batch": index + 1, "total_batches": len(channel_batches()),
        "message": "Next channels are ready." if has_next else "All channels verified successfully.",
        "open_app": not has_next,
    }
